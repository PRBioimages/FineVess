...
def get_itk_array(filenameOrImage,normalize=False):
    ...
    PNG, BMP, DICOM, GIPL, Bio-Rad, LSM,GetArrayFromImage Nifti, Analyze, SDT/SPR (Stimulate),
    Nrrd or VTK images*.'''
...
def convert_to_nii(filenames):
    ...
    return
##我加的
def convert_to_nii_gz(filenames):
    ''' Convert image files to nifti image files.'''
    ...
    return
#我加的
def convert_to_tiff(filenames):
    ''' Convert image files to nifti image files.'''

    for filename in filenames:
        image = get_itk_image(filename)
        nii_filename = os.path.splitext(filename)[0]  + '.tif'
        write_itk_image(image,nii_filename)
...
def get_patch_data(volume4d, divs = (2,2,2,1), offset=(5,5,5,0)):
    patches = []
    shape = volume4d.shape  #（1，640，320，384）
    widths = [ int(s/d) for s,d in zip(shape, divs)]   #（1，64，64，64）
