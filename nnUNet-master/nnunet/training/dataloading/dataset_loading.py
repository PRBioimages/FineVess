...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, skip_blank=False, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        """
        ...
        self.skip_blank=skip_blank  #我加的
    ...
    def get_do_oversample(self, batch_idx):   #搞这个函数就是想说前一半的batch可以不管，后一半的batch必须满足存在前景
    ...
    def generate_train_batch(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)  #随机选了两个volume：    该值是一个array,有两个str类型的元素
        data = np.zeros(self.data_shape, dtype=np.float32)#形状是<class 'tuple'>: (2, 1, 48, 263, 263)
        seg = np.zeros(self.seg_shape, dtype=np.float32)#形状是<class 'tuple'>: (2, 1, 48, 263, 263)
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            if self.get_do_oversample(j):
                ...
                force_fg = False  #在第一个batch时候，是False值
            ...
            if not force_fg:
                ...
                foreground_classes = foreground_classes[foreground_classes > 0]  #得到: 1 (ndarray)
            ...
            if force_fg or self.skip_blank==False:
                bbox_x_ub = bbox_x_lb + self.patch_size[0]
                bbox_y_ub = bbox_y_lb + self.patch_size[1]
                bbox_z_ub = bbox_z_lb + self.patch_size[2]
