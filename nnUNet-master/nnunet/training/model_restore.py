...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    init = info['init'] # <class 'tuple'>: ('/public/yangxiaodu/nnunet/save_data/nnUNet_preprocessed/Task508_other_vessels_raw/nnUNetPlansv2.1_plans_3D.pkl', 0, '/public/yangxiaodu/nnunet/save_data/nnUNet_trained_models/nnUNet/3d_fullres/Task508_other_vessels_raw/nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN__nnUNetPlansv2.1', '/public/yangxiaodu/nnunet/save_data/nnUNet_preprocessed/Task508_other_vessels_raw', True, 1, True, False, True)
    name = info['name'] # 'nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN'
    search_in = join(nnunet.__path__[0], "training", "network_training")  #'/home/xdyang/nnUNet/nnunet/training/network_training'
    tr = recursive_find_python_class([search_in], name, current_module="nnunet.training.network_training") #<class 'nnunet.training.network_training.nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN.nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN'>
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    trainer = restore_model(join(folds[0], "%s.model.pkl" % checkpoint_name), fp16=mixed_precision)  #<nnunet.training.network_training.nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN.nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN object at 0x7f7382175b80>
    trainer.output_folder = folder
    trainer.output_folder_base = folder #'/home/xdyang/nnUNet/save_data/nnUNet_trained_models/nnUNet/3d_fullres/Task508_glioma_vessels_raw/nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN__nnUNetPlansv2.1'
    trainer.update_fold(0)
    trainer.initialize(False)
    all_best_model_files = [join(i, "%s.model" % checkpoint_name) for i in folds] #<class 'list'>: ['/home/xdyang/nnUNet/save_data/nnUNet_trained_models/nnUNet/3d_fullres/Task508_glioma_vessels_raw/nnUNetTrainerV2_loss_ignore_label2_cew_sb_CBAM_decoder_btc_moreDA_BN__nnUNetPlansv2.1/fold_0/model_latest.model']
    print("using the following model files: ", all_best_model_files)
    all_params = [torch.load(i, map_location=torch.device('cpu')) for i in all_best_model_files] #用来加载torch.save() 保存的模型文件。
