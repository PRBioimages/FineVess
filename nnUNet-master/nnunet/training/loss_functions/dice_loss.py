...
import numpy as np
from nnunet.training.loss_functions.improve_crossentropy import Improve1_RobustCrossEntropyLoss
...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    # print('get_tp_fp_fn_tn_shp_x',shp_x)  #torch.Size([2, 3, 12, 56, 56])
    shp_y = gt.shape
    # print('get_tp_fp_fn_tn_shp_y', shp_y)  #torch.Size([2, 1, 12, 56, 56])
    ...
    # print('tp',tp)
    # print('tp_shape', tp.shape)  ##torch.Size([2, 2, 12, 56, 56])
    # print('fp', fp)
    # print('fp_shape', fp.shape) ##torch.Size([2, 2, 12, 56, 56])
    ...
    if mask is not None:
        ...
        # print('mask[:,0].shape', mask[:, 0].shape)  # torch.Size([2, 12, 56, 56])
    if square:
        tp = tp ** 2
        fp = fp ** 2
        fn = fn ** 2
...
class SoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.,ifpen=False,pen_weight=1):
        """
        """
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        if self.ifpen==False:
            ...
            denominator = 2 * tp + self.pen_weight*(fp + fn) + self.smooth
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        try:
            ce_kwargs['weight'] = ce_kwargs['weight'].to(torch.device('cuda:0'))
            self.w = ce_kwargs['weight']
        except KeyError:
    ...
    def forward(self, net_output, target):
        ...
        # print('dc_loss',dc_loss)      #tensor(-0.3524, device='cuda:0', grad_fn=<NegBackward0>)
        if self.log_dice:
            dc_loss = -torch.log(-dc_loss)

        ce_loss = self.ce(net_output, target[:, 0].long()) if self.weight_ce != 0 else 0
        # print('ce_loss',ce_loss)
        # print('ce_loss_shape',ce_loss.shape)  ##([2, 12, 56, 56])
        ...
        if self.ignore_label is not None:
            ...
            if self.w != None:
                c2 = target.sum()
                c1 = mask.sum() - c2
                ce_loss = (ce_loss.sum() + 1e-8) / (c1 * self.w[0] + c2 * self.w[1] + 1e-8)
            else:
                ce_loss = (ce_loss.sum()+1e-8) / (mask.sum()+1e-8)
            # print("ce_loss_sum",ce_loss)
            # '''''''''
...
class DC_and_ICE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs,aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
                 log_dice=False, ignore_label=None):
        """
        CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        ...
        :param weight_dice:
        """
        super(DC_and_ICE_loss, self).__init__()
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            ce_kwargs['reduction'] = 'none'
        self.log_dice = log_dice
        ...
        self.aggregate = aggregate
        self.ce = RobustCrossEntropyLoss(**ce_kwargs)
        # self.ce=Improve1_RobustCrossEntropyLoss()
        try:
            ce_kwargs['weight'] = ce_kwargs['weight'].to(torch.device('cuda:0'))
            self.w = ce_kwargs['weight']
        except KeyError:
    ...
    def forward(self, net_output, target):
        ...
        if self.ignore_label is not None:
            ...
            mask = target != self.ignore_label
            target[~mask] = 0
            # print('target_shape',target.shape)  #([2, 1, 12, 56, 56])
            mask = mask.float()
        else:
            mask = None
        ...
        ce_loss1,mask_v = Improve1_RobustCrossEntropyLoss(net_output,target) if self.weight_ce != 0 else 0  #torch.Size([2, 1, 48, 224, 224])
        # print(ce_loss1)
        # print(ce_loss2)
        ...
        if self.ignore_label is not None:
            ...
            if self.w != None:
                ...
                # c2 = target.sum()
                # c1 = mask.sum() - c2
                ce_loss = (ce_loss1.sum() + 1e-8) / (c1 * self.w[0] + c2 * self.w[1] + 1e-8)
            else:
                ce_loss = (ce_loss1.sum() + 1e-8) / (mask.sum() + 1e-8)
...
class DC_and_focal_loss(nn.Module):
    ...
    def __init__(self, soft_dice_kwargs, focal_kwargs,aggregate="sum", square_dice=False, weight_focal=1, weight_dice=1,
        ...
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            focal_kwargs['size_average'] = 'none'
        self.log_dice = log_dice
        self.weight_dice = weight_dice
        self.weight_focal = weight_focal
        self.aggregate = aggregate
        self.focal=FocalLossV2(apply_nonlin=nn.Softmax(dim=1),**focal_kwargs)
        # self.ce = RobustCrossEntropyLoss(**ce_kwargs)
        # self.ce=Improve1_RobustCrossEntropyLoss()
        # try:
        #     ce_kwargs['weight'] = ce_kwargs['weight'].to(torch.device('cuda:0'))
    ...
    def forward(self, net_output, target):
        ...
        if self.ignore_label is not None:
            ...
            c2 = target.sum()
            c1 = mask.sum() - c2
            focal_loss = (focal_loss.sum() + 1e-8) / (c1 * 0.25 + c2 * 0.75 + 1e-8)
