...
class nnUNetTrainer(NetworkTrainer):
    ...
    def load_dataset(self):
        self.dataset = load_dataset(self.folder_with_preprocessed_data)  #就是把数据集和对应的pkl文件load下来

    def get_basic_generators(self):
        self.load_dataset()
        self.do_split()  #分好数据集放进训练器的dataset_tr还有dataset_val属性里面
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():  #强制之后的内容不进行计算图构建。
            num_classes = output.shape[1]  #类别是3
            output_softmax = softmax_helper(output)
            output_seg = output_softmax.argmax(1)  #返回最大值的索引，若axis=1，表明按行比较，输出每行中最大值的索引
            target = target[:, 0]   #输出每行中的第0列
            axes = tuple(range(1, len(target.shape)))
            tp_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            fp_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            fn_hard = torch.zeros((target.shape[0], num_classes - 1)).to(output_seg.device.index)
            for c in range(1, num_classes):  #计算每个类别的指标 [1,2]
    ...
    def finish_online_evaluation(self):
        ...
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                                           zip(self.online_eval_tp, self.online_eval_fp, self.online_eval_fn)]   #dice计算方法
                               if not np.isnan(i)]
